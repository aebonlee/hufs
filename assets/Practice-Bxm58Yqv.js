import{r as v,j as e}from"./index-CjzEkLNY.js";import{S as W}from"./SEOHead-B9mE2wqv.js";const A=[{id:1,category:"React UI",title:"React 컴포넌트 생성 요청",situation:"교내 특강 신청자 관리 페이지를 만들어야 합니다. 상단에 신청자 입력 폼, 하단에 신청자 목록이 표시되고, 각 항목에 참석 체크박스와 삭제 버튼이 있어야 합니다.",goal:"AI에게 React 컴포넌트를 요청하는 프롬프트를 작성하세요.",keywords:["React","컴포넌트","신청자","명단","입력","목록","체크박스","삭제"],roleKeywords:["개발자","프론트엔드","전문가","역할","엔지니어"],formatKeywords:["코드","TypeScript","CSS","파일","형식","구조","컴포넌트"],exampleAnswer:`너는 React + TypeScript 프론트엔드 전문 개발자야.
교내 특강 신청자 관리 페이지 컴포넌트를 만들어줘.

[기능]
- 상단: 신청자 입력 폼 (이름·학번 입력 + 추가 버튼)
- 하단: 신청자 목록 (참석 체크박스 + 이름 + 삭제 버튼)
- 참석 완료한 신청자는 취소선으로 표시
- useState로 상태 관리
...
- 모던하고 깔끔한 CSS 포함
- 반응형 디자인 (모바일 대응)
- ApplicantItem은 별도 컴포넌트로 분리`},{id:2,category:"API 설계",title:"REST API 엔드포인트 설계",situation:"장학금 신청 관리 시스템의 백엔드 API를 설계해야 합니다. 장학금 신청 CRUD와 학생 인증 기능이 필요하며, Supabase를 백엔드로 사용합니다.",goal:"REST API 엔드포인트를 설계하는 프롬프트를 작성하세요.",keywords:["API","REST","CRUD","엔드포인트","인증","Supabase","설계","JSON"],roleKeywords:["백엔드","개발자","아키텍트","역할","전문가"],formatKeywords:["표","목록","항목","구성","형식","URL","HTTP"],exampleAnswer:`너는 백엔드 API 설계 전문 아키텍트야.
장학금 신청 관리 시스템의 REST API를 설계해줘.

[리소스]
- scholarships: 장학금 신청 (CRUD)
- auth: 학생 인증 (회원가입/로그인/로그아웃)
...
총 10개 이내의 엔드포인트`},{id:3,category:"Streamlit",title:"Streamlit 신청 현황 대시보드 생성",situation:"교환학생 프로그램 지원자 명단 CSV를 업로드하여 학과별 지원 현황, 희망 파견 국가별 분포, 핵심 통계를 시각화하는 Streamlit 대시보드를 만들어야 합니다.",goal:"Streamlit 대시보드를 만드는 프롬프트를 작성하세요.",keywords:["Streamlit","대시보드","데이터","CSV","시각화","차트","지원자","pandas"],roleKeywords:["개발자","Python","전문가","역할","데이터"],formatKeywords:["코드","Python","파일","형식","구조","차트","그래프"],exampleAnswer:`너는 Python Streamlit 전문 개발자야.
교환학생 지원 현황 분석 대시보드를 만들어줘.

[기능]
- CSV 파일 업로드 (st.file_uploader)
- 사이드바: 학기 필터, 단과대학 선택
- 상단: 핵심 통계 카드 (총 지원자 수, 전년 대비, 평균 어학성적)
- 본문: 학과별 지원 현황 막대 차트, 희망 파견 국가별 분포 차트
- 하단: 지원자 명단 테이블 (st.dataframe)

[조건]
- pandas로 데이터 처리
- matplotlib 또는 plotly로 차트 생성
- st.columns로 레이아웃 구성
- st.metric으로 핵심 통계 표시
- 한글 폰트 설정 포함`},{id:4,category:"기획",title:"웹 서비스 요구사항 정의서",situation:"교직원 업무 효율화를 위한 학사 안내 AI 챗봇 웹앱을 기획하고 있습니다. 학생과 교직원이 학칙, 수강신청 FAQ, 졸업요건을 질문하면 Claude가 답변하는 서비스입니다.",goal:"웹 서비스 요구사항 정의서를 요청하는 프롬프트를 작성하세요.",keywords:["요구사항","기획","기능","화면","AI","챗봇","사용자","서비스"],roleKeywords:["기획자","PM","전문가","역할","아키텍트"],formatKeywords:["페이지","구성","형식","목록","분량","목차","표"],exampleAnswer:`너는 웹 서비스 기획 전문가야.
한국외국어대학교 학사 안내 AI 챗봇 웹앱의 요구사항 정의서를 작성해줘.

[서비스 개요]
- 대상: 재학생 및 교직원 (약 20,000명)
- 목적: 학칙, 수강신청 FAQ, 졸업요건 AI 검색/답변
- 기술: React + Supabase + Claude API
...
[조건]
- A4 3페이지 이내
- 비전공 행정직원도 이해할 수 있는 용어 사용`},{id:5,category:"Python 분석",title:"Python 데이터 분석 코드 요청",situation:"학생 민원 접수 이력 데이터(Excel)를 분석하여 민원 유형별 빈도, 월별 추이, 처리기간 분석을 수행하는 Python 코드가 필요합니다.",goal:"Python 데이터 분석 코드를 요청하는 프롬프트를 작성하세요.",keywords:["Python","분석","데이터","pandas","Excel","민원","통계","시각화"],roleKeywords:["데이터","분석가","전문가","역할","Python"],formatKeywords:["코드","Python","파일","형식","그래프","차트","출력"],exampleAnswer:`너는 Python 데이터 분석 전문가야.
학생 민원 접수 이력 데이터를 분석하는 코드를 작성해줘.

[데이터 구조]
- 파일: Excel (.xlsx)
- 컬럼: 접수일시, 부서, 민원유형, 학과, 처리기간(일), 처리결과
...
- 한글 폰트 설정 포함
- 각 분석 결과를 PNG로 저장
- 요약 통계를 print로 출력`},{id:6,category:"Claude API",title:"Claude API 연동 Streamlit 챗봇",situation:"Streamlit 앱에 Claude API를 연동하여 유학생 생활·비자 관련 질의응답을 처리하는 AI 챗봇을 만들어야 합니다. 대화 이력을 유지하고, 국제교류처 행정 안내 전문가 역할을 수행해야 합니다.",goal:"Claude API를 연동한 Streamlit 챗봇 코드를 요청하는 프롬프트를 작성하세요.",keywords:["Claude","API","Streamlit","챗봇","AI","대화","연동","유학생"],roleKeywords:["AI","개발자","전문가","역할","Python"],formatKeywords:["코드","Python","파일","형식","구조","Streamlit","설정"],exampleAnswer:`너는 Python + Streamlit + Claude API 전문 개발자야.
국제교류처 유학생 안내용 AI 챗봇 Streamlit 앱을 만들어줘.

[기능]
- Claude API 연동 (anthropic 라이브러리)
- st.chat_input / st.chat_message로 대화 UI
- st.session_state로 대화 이력 관리
- System Prompt: "너는 한국외국어대학교 국제교류처 행정 안내 전문가야"
